...
func (x *User) PerformValidations() {
...
func (x *User) UpdateFromAttrs(attrs UserAttrs) {
	if attrs.NameFirst != "" {
		x.NameFirst = attrs.NameFirst
	}
	if attrs.NameLast != "" {
		x.NameLast = attrs.NameLast
	}
	if attrs.Email != "" {
		x.Email = attrs.Email
	}
	if attrs.Password != "" {
		x.Password = attrs.Password
	}
	if attrs.PasswordConfirmation != "" {
		x.PasswordConfirmation = attrs.PasswordConfirmation
	}
}
